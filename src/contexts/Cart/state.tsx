...
const CartState = (props: any) => {
    ...
    const updateCartData: Function = () => {
        request(REQUEST_TYPE.GET, `https://qckbuyproduct.azurewebsites.net/api/v1/user/cart`, cookies.token).then(data => {
            if(data?.success){
    ...
    const updateProductInCart: Function = bigPromise(async (productId: string, toIncrement: boolean) => {
        try{
            const data = await request(
                ...
                `https://qckbuyproduct.azurewebsites.net/api/v1/user/cart/${productId}`, 
                cookies.token
            );
            if(data?.success){
    ...
    const deleteCart = async () => {
        try{
            const data = await request(
                REQUEST_TYPE.DELETE, 
                `https://qckbuyproduct.azurewebsites.net/api/v1/user/cart`,
                cookies.token
            );
            if(data?.success){
                setCart([]);

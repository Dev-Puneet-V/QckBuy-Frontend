...
const orderAcceptHandler = async (orderId: string, currRow: any) => {
    const data = await request(
        REQUEST_TYPE.POST,
        `https://qckbie-production.up.railway.app//order/accept/${orderId}`,
        cookies.token
    )
    if(data?.success){
...
const deliveryHandler = async (orderId: string,currRow: any) => {
    const data = await request(
        REQUEST_TYPE.GET,
        `https://qckbie-production.up.railway.app//order/delivery/initiate/${orderId}`,
        cookies.token
    );
    if(data?.success){
        setOrderId(orderId);
    ...
    const availableOrdersFunction = () => {
        getData('https://qckbie-production.up.railway.app//dashboard/employee/available-order', 1);     
    }
    const acceptedOrdersFunction = () => {
        getData('https://qckbie-production.up.railway.app//dashboard/employee/accepted-order', 2);   
    }
    const historyFunction = () => {
        getData('https://qckbie-production.up.railway.app//dashboard/employee/history', 3);       
    }
    ...
    const otpSubmitHandler = async () => {
        ...
        const data = await request(
            REQUEST_TYPE.POST,
            `https://qckbie-production.up.railway.app//order/delivery/proceed/${orderId}`,
            cookies.token,
            {
                "otp": otp
            }

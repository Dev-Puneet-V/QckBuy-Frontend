  ...
  const isMobileView = useMediaQuery(theme.breakpoints.down('sm'));
    ...
    const invoiceDownloadHandler = async () => {
        ...
        const invoice = await request(
            REQUEST_TYPE.GET, 
            `https://qckbuyproduct.azurewebsites.net/api/v1/order/${paymentId}/invoice/true`, 
    ...
    if (result.error) {
      ...
      if(result.error.type !== 'validation_error'){
        ...
        if(result?.paymentIntent?.status === "succeeded"){
            ...
            const postPaymentResponse = await request(REQUEST_TYPE.GET, `https://qckbuyproduct.azurewebsites.net/api/v1/payment/stripe/paymentSuccess/${props.paymentIntentToken}`, cookies.token);
            ...
            const postPaymentResponse = await request(REQUEST_TYPE.GET, `https://qckbuyproduct.azurewebsites.net/api/v1/payment/stripe/paymentFailure/${props.paymentIntentToken}`, cookies.token);
            // const postPaymentData = await postPaymentResponse.json();
            if(postPaymentResponse.success) {
                setPaymentStatus(PAYMENT_STATUS.FAILED);                
            }
...
const Component = () => {
    ...
    useEffect(() => { 
        const processor = async () => {
            setRetry(retry + 1);
            const paymentIntentResponse = await request(REQUEST_TYPE.POST, "https://qckbuyproduct.azurewebsites.net/api/v1/payment/stripe/initiatePayment", cookies.token);

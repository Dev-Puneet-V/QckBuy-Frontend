  ...
  const isMobileView = useMediaQuery(theme.breakpoints.down('sm'));
    ...
    const invoiceDownloadHandler = async () => {
        ...
        const invoice = await request(
            REQUEST_TYPE.GET, 
            `${process.env.REACT_APP_API_BASE_URL}/order/${paymentId}/invoice/true`, 
    ...
    if (result.error) {
      ...
      if(result.error.type !== 'validation_error'){
        ...
        if(result?.paymentIntent?.status === "succeeded"){
            ...
            const postPaymentResponse = await request(REQUEST_TYPE.GET, `${process.env.REACT_APP_API_BASE_URL}/payment/stripe/paymentSuccess/${props.paymentIntentToken}`, cookies.token);
            ...
            const postPaymentResponse = await request(REQUEST_TYPE.GET, `${process.env.REACT_APP_API_BASE_URL}/payment/stripe/paymentFailure/${props.paymentIntentToken}`, cookies.token);
            // const postPaymentData = await postPaymentResponse.json();
            if(postPaymentResponse.success) {
                setPaymentStatus(PAYMENT_STATUS.FAILED);                
            }
...
const Component = () => {
    ...
    useEffect(() => { 
        const processor = async () => {
            setRetry(retry + 1);
            const paymentIntentResponse = await request(REQUEST_TYPE.POST, "${process.env.REACT_APP_API_BASE_URL}/payment/stripe/initiatePayment", cookies.token);
